...
# ── Feature builder (inline, no API dependency) ───────────────────────────────
...
SERVICE_LINE_MAP = {
    "Asia-EU":0,"Med-India":1,"Asia-Med":2,"Intra-Med":3,"Red-Sea-Med":4,
    "North-EU":5,"West-Africa":6,"East-Africa":7,"Americas":8,"Adriatic":9,
}

def teu_class(teu):
    if teu < 3000:   return "small"
    if teu < 8000:   return "medium"
    if teu < 14000:  return "large"
    return "ultra"

def berth_zone(berth_id):
    num = int(berth_id[1:]) if len(berth_id) > 1 else 1
    if num <= 5:   return "north"
    if num <= 10:  return "central"
    if num <= 15:  return "south"
    return "industrial"
...
def build_features(vessel_type, teu_capacity, dwt, loa, draft,
    ...
    f[10] = is_night
    f[11] = is_peak
    f[12] = int(month in [6,7,8])   # summer
    f[13] = int(month in [12,1,2])  # winter
    f[14] = 0  # is_jewish_holiday (default 0)
    f[15] = arrivals_6h
    f[16] = arrivals_12h
    # vessel (17-25)
    f[17] = teu_capacity
    ...
    f[35] = 0  # port_avg_wait_7d (default)
    f[36] = 0  # port_avg_wait_30d
    f[37] = 0  # vessel_prev_wait
    f[38] = 0  # vessel_call_count
    # weather (39-41)
    f[39] = weather_wind
    f[40] = int(weather_wind > 30)
    f[41] = berth_comp  # stored berth_competition
    # cargo (42-43)
...
with tab1:
    ...
    with col2:
        ...
        teu_loaded      = st.slider("TEU Loaded",     0, max(teu_capacity,1), max(teu_capacity,1)//2,
                                     disabled=vessel_type != "CONTAINER")
        teu_discharged  = st.slider("TEU Discharged", 0, max(teu_capacity,1), max(teu_capacity,1)//2,
                                     disabled=vessel_type != "CONTAINER")
    ...
    if predict_btn:
        ...
        wait, ci, occ_class, occ_probs, cong_prob, cong_flag = predict(m1, m2, m3, X)

        # Recommended berth (simple rule: lowest-competition berth in port)
...
with tab2:
    ...
    with col_r:
        if run_fc:
            hours, utils, classes = [], [], []
            for h in range(24):
                eta_dt_fc = datetime(fc_date.year, fc_date.month, fc_date.day, h)
                bc_h = bc_val * (1.2 if 8 <= h <= 18 else 0.7)
                X_fc = build_features(
                    "CONTAINER", 8000, 80000, 250, 12.0,
                    port_b, berth_b, "Asia-EU", eta_dt_fc,
                    3, 80000.0, 4000, 4000, 8.0, bc_h,
                    5, 10, 20, 5
                )
                wait_h, _, occ_cls, occ_p, _, _ = predict(m1, m2, m3, X_fc)
